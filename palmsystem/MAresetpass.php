...
$encoded_IDadmin = $_GET['Get'];
$decoded_IDadmin = base64_decode($encoded_IDadmin);
...
//Get admin table ID USE IS decoded_IDadmin
$sqlIDadmin = "SELECT * FROM `admin` WHERE `IDadmin` = '$decoded_IDadmin'";
$resultIDadmin = $con->query($sqlIDadmin);

if($resultIDadmin->num_rows > 0){
    while ($rowMA = $resultIDadmin->fetch_assoc()) {
        $IDadmin = $rowMA['IDadmin'];
        $Afname = $rowMA['Afname'];
        $Alname = $rowMA['Alname'];
        $Amname = $rowMA['Amname'];
        $Ausername = $rowMA['Ausername'];
        $Apassword = $rowMA['Apassword'];
    }
}
?>
...
<head>
    ...
    <link rel="stylesheet" href="systemcss/MAresetpass.css">

</head>

<body class="bg-image">
    
    <!--Invalid Token Design-->
    ...
    <div class="resetpassform d-none container mt-5 mb-5 pt-5 d-flex justify-content-center align-items-center main">
        <div class="row mt-3 row rowone shadow bg-light">
            ...
            <div class="col-7 right">
                ...
                <form id="formreset">
                    <div class="emptyfields d-none mb-2 text-center" style="color: red; font-size:12px;"></div>
                    <p class="text-center text-secondary mt-3">Username: <?php echo $Ausername?></p>

                    <!--FOR PASSWORD-->
                    <div class="form-group mb-3">
                        <label class="inputLogLabels text-secondary mb-1" for="Apassword">New Password </label>
                            <div class="PasswordReset input-group">
                                <input type="password" name="Apassword" class="form-control" id="Apassword" placeholder="Enter New Password" oninput="this.value = this.value.replace(/\s+/g, '');">
                                <label class="bi bi-eye input-group-text" id="Apassword-toggle-icon" style="color:#827d7d;">
                            </div>
                    ...
                    <div class="form-group">
                        <label class="inputLogLabels text-secondary mb-1" for="Acpassword">Confirm New Password</label>
                        <input type="password" class="form-control" name="Acpassword" id="Acpassword" placeholder="Enter New Password" oninput="this.value = this.value.replace(/\s+/g, '');">
...
//FOR FORM SCRIPT
  $(document).on('submit', '#formreset', function (e) {
        ...
        var MAformDataReset = new FormData(this);
        MAformDataReset.append("btnreset", true);
        MAformDataReset.append("IDadmin", "<?php echo $IDadmin; ?>");


        $.ajax({
            type: "POST",
            url: "Function/MAFunctionresetpass.php",
            data: MAformDataReset,
            processData: false,
            contentType: false,
