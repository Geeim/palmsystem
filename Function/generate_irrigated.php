...
$Mayor = $_GET['Mayor'];
    ...
    $stmtcheckPlanting = $con->prepare("SELECT * FROM `planting` WHERE  `landtype` = 1 AND `month` = ? AND `range_date` = ? AND `year` = ?");
    ...
    $stmtcheckHarvesting = $con->prepare("SELECT * FROM `harvesting` WHERE `landtype` = 1 AND `month` = ? AND `range_date` = ? AND `year` = ?");
...
// WORKSHEET FOR HARVESTING (sheet1)
    ...
    //MERGE CELL
        ...
        $sheet1->mergeCells('E7:AQ7'); //IRRIGATED
    ...
    //VALUE DESIGN
        ...
        $sheet1->setCellValue('E7', 'IRRIGATED');
    ...
    //COLOR
        ...
        $sheet1->getStyle('E7')->getFill()->getStartColor()->setRGB('FFFF00');  // IRRIGATED COLOR
    ...
    //SUM COMPUTATION - LINE 35
        ...
        // OK
        $sheet2->mergeCells('D7:AC7'); //IRRIGATED
    ...
    // VALUE
        ...
        $sheet2->setCellValue('D7', 'IRRIGATED');
    ...
    //COLOR
        ...
        $sheet2->getStyle('D7')->getFill()->getStartColor()->setRGB('FFFF00');  // IRRIGATED COLOR
    ...
    $filename ='IRRIGATED ' . $monthName . ' ' . $range_date . ' ' . $year . ' REPORT';
