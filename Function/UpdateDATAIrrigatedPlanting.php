...
$response = [];  // Array to store response messages
    ...
    if (isset($_POST['updatedDataNPR'])) {
        ...
        if ($updatedDataNPR === null) {
            ...
            foreach ($updatedDataNPR as $row) {
                ...
                b.BarangayName = ? AND landtype = 1 AND p.seed_name = 1 AND`project_type` = 1 AND p.year = ? AND p.month = ? AND p.range_date = ?;";

                // Update for Registered Seed
                $query_registered = " UPDATE planting p JOIN barangay b ON p.barangay = b.IDbarangay 
                SET p.area_planted = ?,  p.no_farmers = ? WHERE 
                b.BarangayName = ? AND landtype = 1 AND p.seed_name = 2 AND`project_type` = 1 AND p.year = ? AND p.month = ? AND p.range_date = ?;";

                // Update for Certified Seed
                $query_certified = " UPDATE planting p JOIN barangay b ON p.barangay = b.IDbarangay 
                SET p.area_planted = ?,  p.no_farmers = ? WHERE 
                b.BarangayName = ? AND landtype = 1 AND p.seed_name = 3 AND`project_type` = 1 AND p.year = ? AND p.month = ? AND p.range_date = ?;";
    ...
    if (isset($_POST['updatedDataRCEF'])) {
        ...
        if ($updatedDataRCEF === null) {
            ...
            foreach ($updatedDataRCEF as $row) {  // This line was missing a closing brace.
                ...
                $query_hybrid = " UPDATE planting p JOIN barangay b ON p.barangay = b.IDbarangay 
                SET p.area_planted = ?,  p.no_farmers = ? WHERE 
                b.BarangayName = ? AND landtype = 1 AND p.seed_name = 1 AND`project_type` = 2 AND p.year = ? AND p.month = ? AND p.range_date = ?;";
    
                // Update for Registered Seed
                $query_registered = " UPDATE planting p JOIN barangay b ON p.barangay = b.IDbarangay 
                SET p.area_planted = ?,  p.no_farmers = ? WHERE 
                b.BarangayName = ? AND landtype = 1 AND p.seed_name = 2 AND`project_type` = 2 AND p.year = ? AND p.month = ? AND p.range_date = ?;";
    
                // Update for Certified Seed
                $query_certified = " UPDATE planting p JOIN barangay b ON p.barangay = b.IDbarangay 
                SET p.area_planted = ?,  p.no_farmers = ? WHERE 
                b.BarangayName = ? AND landtype = 1 AND p.seed_name = 3 AND`project_type` = 2 AND p.year = ? AND p.month = ? AND p.range_date = ?;";
        ...
        if ($updatedDataOWNOTHERS === null) {
            ...
            foreach ($updatedDataOWNOTHERS as $row) {
                ...
                $range_date = $row['range_date']; 
                 ...
                 $query_hybrid = " UPDATE planting p JOIN barangay b ON p.barangay = b.IDbarangay 
                 SET p.area_planted = ?,  p.no_farmers = ? WHERE 
                 b.BarangayName = ? AND landtype = 1 AND p.seed_name = 1 AND`project_type` = 3 AND p.year = ? AND p.month = ? AND p.range_date = ?;";
     
                 // Update for Registered Seed
                 $query_registered = " UPDATE planting p JOIN barangay b ON p.barangay = b.IDbarangay 
                 SET p.area_planted = ?,  p.no_farmers = ? WHERE 
                 b.BarangayName = ? AND landtype = 1 AND p.seed_name = 2 AND`project_type` = 3 AND p.year = ? AND p.month = ? AND p.range_date = ?;";
     
                 // Update for Certified Seed
                 $query_certified = " UPDATE planting p JOIN barangay b ON p.barangay = b.IDbarangay 
                 SET p.area_planted = ?,  p.no_farmers = ? WHERE 
                 b.BarangayName = ? AND landtype = 1 AND p.seed_name = 3 AND`project_type` = 3 AND p.year = ? AND p.month = ? AND p.range_date = ?;";
    ...
    if (isset($_POST['updatedDataINFORMAL'])) {
        ...
        if ($updatedDataINFORMAL === null) {
            ...
            foreach ($updatedDataINFORMAL as $row) {
                ...
                SET p.area_planted = ?,  p.no_farmers = ? WHERE 
                b.BarangayName = ? AND landtype = 1 AND p.seed_name = 4 AND `project_type` is NULL AND p.year = ? AND p.month = ? AND p.range_date = ?;";
    
                // Update for TAGGED Seed
                $query_tagged = " UPDATE planting p JOIN barangay b ON p.barangay = b.IDbarangay 
                SET p.area_planted = ?,  p.no_farmers = ? WHERE 
                b.BarangayName = ? AND landtype = 1  AND p.seed_name = 5 AND `project_type` is NULL AND p.year = ? AND p.month = ? AND p.range_date = ?;";
    
                // Update for TRADITONAL Seed
                $query_traditional = " UPDATE planting p JOIN barangay b ON p.barangay = b.IDbarangay 
                SET p.area_planted = ?,  p.no_farmers = ? WHERE 
                b.BarangayName = ? AND landtype = 1 AND p.seed_name = 6 AND `project_type` is NULL AND  p.year = ? AND p.month = ? AND p.range_date = ?;";
    ...
    if (isset($_POST['updatedDataFSS'])) {
        ...
        if ($updatedDataFSS === null) {
            ...
            SET p.area_planted = ?,  p.no_farmers = ? WHERE 
            b.BarangayName = ? AND landtype = 1 AND p.seed_name IS NULL AND `project_type` is NULL AND p.year = ? AND p.month = ? AND p.range_date = ?;";
