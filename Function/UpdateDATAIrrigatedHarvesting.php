    ...
    if ($updatedDataNPRHarvesting === null) {
        ...
        foreach ($updatedDataNPRHarvesting as $row) {
            ...
            $range_date = $row['range_date']; 
                ...
                b.BarangayName = ? AND landtype = 1 AND h.seed_name = 1 AND`project_type` = 1 AND h.year = ? AND h.month = ? AND h.range_date = ?;";
                
                // Update for Registered Seed
                $query_registered = "UPDATE harvesting h JOIN barangay b ON h.barangay = b.IDbarangay 
                SET h.area_harvested = ?,  h.average_yield = ?, h.production = ? WHERE 
                b.BarangayName = ? AND landtype = 1 AND h.seed_name = 2 AND`project_type` = 1 AND h.year = ? AND h.month = ? AND h.range_date = ?;";
                
                // Update for Certified Seed
                $query_certified = "UPDATE harvesting h JOIN barangay b ON h.barangay = b.IDbarangay 
                SET h.area_harvested = ?,  h.average_yield = ?, h.production = ? WHERE 
                b.BarangayName = ? AND landtype = 1 AND h.seed_name = 3 AND`project_type` = 1 AND h.year = ? AND h.month = ? AND h.range_date = ?;";
    ...
    if ($updatedDataRCEFHarvesting === null) {
        ...
        foreach ($updatedDataRCEFHarvesting as $row) {
            ...
            $range_date = $row['range_date']; 
                ...
                $query_hybrid = "UPDATE harvesting h JOIN barangay b ON h.barangay = b.IDbarangay 
                SET h.area_harvested = ?,  h.average_yield = ?, h.production = ? WHERE 
                b.BarangayName = ? AND landtype = 1 AND h.seed_name = 1 AND`project_type` = 2 AND h.year = ? AND h.month = ? AND h.range_date = ?;";
                
                // Update for Registered Seed
                $query_registered = "UPDATE harvesting h JOIN barangay b ON h.barangay = b.IDbarangay 
                SET h.area_harvested = ?,  h.average_yield = ?, h.production = ? WHERE 
                b.BarangayName = ? AND landtype = 1 AND h.seed_name = 2 AND`project_type` = 2 AND h.year = ? AND h.month = ? AND h.range_date = ?;";
                
                // Update for Certified Seed
                $query_certified = "UPDATE harvesting h JOIN barangay b ON h.barangay = b.IDbarangay 
                SET h.area_harvested = ?,  h.average_yield = ?, h.production = ? WHERE 
                b.BarangayName = ? AND landtype = 1 AND h.seed_name = 3 AND`project_type` = 2 AND h.year = ? AND h.month = ? AND h.range_date = ?;";
    ...
    if ($updatedDataOWNOTHERSHarvesting === null) {
        ...
        foreach ($updatedDataOWNOTHERSHarvesting as $row) {
            ...
            $range_date = $row['range_date']; 
                ...
                $query_hybrid = "UPDATE harvesting h JOIN barangay b ON h.barangay = b.IDbarangay 
                SET h.area_harvested = ?,  h.average_yield = ?, h.production = ? WHERE 
                b.BarangayName = ? AND landtype = 1 AND h.seed_name = 1 AND`project_type` = 3 AND h.year = ? AND h.month = ? AND h.range_date = ?;";
                
                // Update for Registered Seed
                $query_registered = "UPDATE harvesting h JOIN barangay b ON h.barangay = b.IDbarangay 
                SET h.area_harvested = ?,  h.average_yield = ?, h.production = ? WHERE 
                b.BarangayName = ? AND landtype = 1 AND h.seed_name = 2 AND`project_type` = 3 AND h.year = ? AND h.month = ? AND h.range_date = ?;";
                
                // Update for Certified Seed
                $query_certified = "UPDATE harvesting h JOIN barangay b ON h.barangay = b.IDbarangay 
                SET h.area_harvested = ?,  h.average_yield = ?, h.production = ? WHERE 
                b.BarangayName = ? AND landtype = 1 AND h.seed_name = 3 AND`project_type` = 3 AND h.year = ? AND h.month = ? AND h.range_date = ?;";
    ...
    if ($updatedDataINFORMALHarvesting === null) {
        ...
        foreach ($updatedDataINFORMALHarvesting as $row) {
            ...
            $range_date = $row['range_date']; 
                ...
                SET h.area_harvested = ?,  h.average_yield = ?, h.production = ? WHERE 
                b.BarangayName = ? AND landtype = 1 AND h.seed_name = 4 AND`project_type` IS NULL AND h.year = ? AND h.month = ? AND h.range_date = ?;";
                
                // Update for TAGGED Seed
                $query_tagged = "UPDATE harvesting h JOIN barangay b ON h.barangay = b.IDbarangay 
                SET h.area_harvested = ?,  h.average_yield = ?, h.production = ? WHERE 
                b.BarangayName = ? AND landtype = 1 AND h.seed_name = 5 AND`project_type` IS NULL AND h.year = ? AND h.month = ? AND h.range_date = ?;";
                
                // Update for Traditional Seed
                $query_traditional = "UPDATE harvesting h JOIN barangay b ON h.barangay = b.IDbarangay 
                SET h.area_harvested = ?,  h.average_yield = ?, h.production = ? WHERE 
                b.BarangayName = ? AND landtype = 1 AND h.seed_name = 6 AND`project_type` IS NULL AND h.year = ? AND h.month = ? AND h.range_date = ?;";
    ...
    if ($updatedDataFSSHarvesting === null) {
        ...
        foreach ($updatedDataFSSHarvesting as $row) {
            ...
            $range_date = $row['range_date']; 
                ...
                SET h.area_harvested = ?,  h.average_yield = ?, h.production = ? WHERE 
                b.BarangayName = ? AND landtype = 1 AND h.seed_name IS NULL AND`project_type` IS NULL AND h.year = ? AND h.month = ? AND h.range_date = ?;";
