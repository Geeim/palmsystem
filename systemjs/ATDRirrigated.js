if (!window.barangays) {
    ...
    // MODAL SET UP
        ...
        SetUpForm.addEventListener('submit', function(event) {
            ...
            if (this.checkValidity()) {
                ...
                $.ajax({
                    type: 'POST',
                    url: 'Function/DRCheckSetUpModal.php',
                    data: { 
    ...
    SubmitDataHarvestedID.addEventListener('click', function() {
        ...
        if (CheckRows) {
            $.ajax({
                url: 'Function/DRHarvestingFunction.php',
                type: 'POST',
...
//SUBMIT THE INPUT PLANTING
    ...
    SubmitDataPlantedID.addEventListener('click', function() {
        ...
        if (CheckRows) {
            $.ajax({
                url: 'Function/DRPlantingFunction.php',
                type: 'POST',
    ...
    //PLANTING PART IRRIGATED
    //FETCH IRRIGATED PLANTING VIEW AND UPDATE
        ...
        function ViewPlanting(year, month, range_date) {
            $.ajax({
                url: 'Function/DATAIrrigatedViewPlanting.php',  // PHP script to fetch data
                type: 'POST',
                data: {
                    year: year,
                    month: month,
                ...
                success: function(response) {
                    ...
                    $('#saveChanges').on('click', function() {
                        ...
                        $.ajax({
                            url: 'Function/UpdateDATAIrrigatedPlanting.php',  // PHP script to save updated data
                            type: 'POST',
...
//HARVESTING PART IRRIGATED
    //FETCH IRRIGATED HARVESTING VIEW AND UPDATE
    ...
    function ViewHarvesting(year, month, range_date) {
        $.ajax({
            url: 'Function/DATAIrrigatedViewHarvesting.php',  // PHP script to fetch data
            type: 'POST',
            data: {
                year: year,
                month: month,
            ...
            //GET THE AVERAGE YIELD
                ...
                //GET UPDATE
                    ...
                    $('#saveChangesHarvesting').on('click', function() {
                        ...
                        $.ajax({
                            url: 'Function/UpdateDATAIrrigatedHarvesting.php',  // PHP script to save updated data
                            type: 'POST',
                            data: dataToSend,  // Stringify the updatedData before sending
                            dataType: 'json',  // Expecting a JSON response
                            success: function(response) {
...
}//CLOSE THE WINDOW CHECK CONDITION



$(document).on('submit', '#exportirrigated', function (e) {
    e.preventDefault();

    var ExportIrrigatedForm = new FormData(this);

    $.ajax({
        type: "POST",
        url: "Function/checkexist_irrigated.php",  // URL of the PHP script that processes the form data
        data: ExportIrrigatedForm,
        processData: false,
        contentType: false,
        success: function (response) {
            var res = JSON.parse(response);
            ...
            if (res.status == 'SUCCESS') {
               $('#btnexportIrrigated').prop('disabled', true).text('Downloading...');
               $('#monthexportIrrigated').prop('disabled', true);
               ...
               setTimeout(function() {
                    ...
                    $('#monthexportIrrigated').prop('disabled', false);
                    $('#rangeDateExportIrrigated').prop('disabled', false);
                    $('#yearExportIrrigated').prop('disabled', false);
                    $('#mayorsnameexportIrrigated').prop('disabled', false);
